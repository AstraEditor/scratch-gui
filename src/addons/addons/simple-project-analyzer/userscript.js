...
export default function ({ addon, msg, console }) {
    ...
    class SimpleProjectAnalyzer {
      ...
      `;
        ...
        updateSidebarStats(analysis) {
            ...
            const stats = [
                ...
                {
                    label: msg("effective-blocks"),
                    value: analysis.effectiveBlocks,
                },
                {
                    label: msg("function-definitions"),
                    value: analysis.functionDefinitions,
                },
      ...
      `,
        ...
        displaySidebarCodeTypesChart(analysis) {
            ...
            Object.keys(analysis.codeTypes)
                .sort()
                .forEach((category) => {
                    if (!standardOrder.includes(category)) {
                        sortedCodeTypes[category] =
                            analysis.codeTypes[category];
                    }
                });
        ...
        displaySidebarExtensions(extensions) {
            const extensionList = document.getElementById(
                "saSAPAExtensionsList",
            );
            ...
            targets.forEach((target) => {
                ...
                Object.values(blocks).forEach((block) => {
                    if (block.opcode && !block.shadow) {
                        ...
                        const category = this.getBlockCategory(
                            block.opcode,
                            extensionNameMap,
                        );
                        analysis.codeTypes[category] =
                            (analysis.codeTypes[category] || 0) + 1;
                    }
                });
            });
        }
        ...
        getBlockCategory(opcode, extensionNameMap = {}) {
            ...
            if (opcode.startsWith("procedures_"))
                return msg("custom-functions", "自定义函数");
            if (opcode.startsWith("argument_"))
                return msg("custom-functions", "自定义函数");
        ...
        isStandardBlock(opcode) {
            ...
            return standardCategories.some((category) =>
                opcode.startsWith(category),
            );
        }
        ...
        analyzeExtensions(analysis, projectData) {
            ...
            extensions.forEach((ext) => {
                ...
                const extensionInfo = {
                    ...
                    blocks: extensionBlocks.filter(
                        (block) => block.extensionId === ext,
                    ),
                };
            ...
            // 检查画笔扩展使用情况
            const penBlocks = extensionBlocks.filter(
                (block) => block.extensionId === "pen",
            );
            ...
            targets.forEach((target) => {
                ...
                Object.values(blocks).forEach((block) => {
                    if (block.opcode && !this.isStandardBlock(block.opcode)) {
                        const extensionId = this.getExtensionIdFromOpcode(
                            block.opcode,
                        );
        ...
        analyzeSprites(analysis, projectData) {
            ...
            targets.forEach((target) => {
                ...
                if (target.variables) {
                    analysis.variableCount += Object.keys(
                        target.variables,
                    ).length;
                }
                if (target.lists) {
            ...
            targets.forEach((target) => {
                ...
                Object.values(blocks).forEach((block) => {
                    if (block.opcode && !block.shadow && block.topLevel) {
                        ...
                        if (
                            vm.runtime.getIsHat(block.opcode) ||
        ...
        calculateDrScratchScores(projectData) {
            ...
            const hasCustomBlocks = procedureBlockTypes.has(
                "procedures_definition",
            );
            const hasClones =
                controlBlockTypes.has("control_create_clone_of") ||
                controlBlockTypes.has("control_start_as_clone");
            ...
            const hasClickEvents = eventBlockTypes.has(
                "event_whenthisspriteclicked",
            );
            const hasMessageEvents = eventBlockTypes.has(
                "event_whenbroadcastreceived",
            );
            const hasCloneEvents = controlBlockTypes.has(
                "control_create_clone_of",
            );
            const hasSensorEvents = eventBlockTypes.has(
                "event_whengreaterthan",
            );
            const hasBackdropEvents = eventBlockTypes.has(
            ...
            const hasLogicOps =
                operatorBlockTypes.has("operator_and") ||
            ...
            const hasReceiveMessage = eventBlockTypes.has(
                "event_whenbroadcastreceived",
            );
            const hasStopAll = controlBlockTypes.has("control_stop_all");
            const hasStopThis = controlBlockTypes.has(
                "control_stop_this_script",
            );
            const hasStopOther = controlBlockTypes.has(
                "control_stop_other_scripts_in_sprite",
            );
            const hasWaitUntil = controlBlockTypes.has("control_wait_until");
            const hasBackdropChange =
                looksBlockTypes.has("looks_nextbackdrop") ||
                looksBlockTypes.has("looks_switchbackdropto");
            const hasBroadcastAndWait = eventBlockTypes.has(
            ...
            const hasRepeat =
                controlBlockTypes.has("control_repeat") ||
                controlBlockTypes.has("control_forever");
            const hasRepeatUntil = controlBlockTypes.has(
            ...
            // 基于不同的交互积木块种类
            const hasGreenFlagEvent = eventBlockTypes.has(
                "event_whenflagclicked",
            );
            const hasKeyPressedEvent = eventBlockTypes.has(
                "event_whenkeypressed",
            );
            const hasSpriteClickedEvent = eventBlockTypes.has(
                "event_whenthisspriteclicked",
            );
            const hasAskWait = sensingBlockTypes.has("sensing_askandwait");
            const hasMouseBlocks =
                sensingBlockTypes.has("sensing_mousedown") ||
                sensingBlockTypes.has("sensing_mousex") ||
                sensingBlockTypes.has("sensing_mousey");
            const hasSensorGreater = eventBlockTypes.has(
                "event_whengreaterthan",
            );
            const hasVideo = Array.from(blockTypes).some((type) =>
                type.startsWith("video_"),
            );
            const hasAudioInteraction =
                soundBlockTypes.has("sound_playuntildone") ||
            ...
            // 基于不同的数据积木块种类
            const hasSpriteModifiers =
                motionBlockTypes.size > 0 ||
                looksBlockTypes.size > 0 ||
                soundBlockTypes.size > 0;
            const hasVariableOperations =
                variableNames.size > 0 && dataBlockTypes.size > 0;
            const hasListOperations =
                listNames.size > 0 &&
                Array.from(dataBlockTypes).some((type) =>
                    type.includes("list"),
                );
        ...
        calculateMathLogicScores(projectData) {
            ...
            const scores = {
                [msg("operation-complexity", "Operation Complexity")]:
                    operatorCount,
      ...
      `;
        ...
        updateStats(analysis) {
            ...
            const totalExtensionBlocks = analysis.extensions.reduce(
                (sum, ext) => sum + ext.blocks.length,
                0,
            );
      ...
      `;
        ...
        displayCodeTypeChart(analysis) {
            ...
            Object.keys(extensionCategories)
                .sort()
                .forEach((category) => {
            ...
            const assignedColors = sortedLabels.map((label, index) => {
                if (categoryColors[label]) {
                    ...
                    // 检查是否是具有自定义颜色的扩展
                    const extension = analysis.extensions.find(
                        (ext) => ext.name === label,
                    );
        ...
        displayDrScratchScores(scores) {
            ...
            // 更新总分
            document.getElementById("saDrScratchTotalScore").textContent =
                totalScore;
            ...
            document.getElementById("saDrScratchScoreLevel").textContent =
                `${msg("evaluation-level", "评估等级")}：${level}`;

            // 更新评分详情
            const detailsHTML = labels
                .map(
                    (label) => `
      ...
      `,
            ...
            const detailsContainer =
                document.getElementById("saDrScratchDetails");
            const summaryHTML = detailsContainer.querySelector(
        ...
        displayMathLogicScores(scores) {
            ...
            // 更新总分
            document.getElementById("saMathTotalScore").textContent =
                totalScore;
            ...
            document.getElementById("saMathScoreLevel").textContent =
                `${msg("evaluation-level", "评估等级")}：${level}`;
      ...
      `,
            ...
            const detailsContainer =
                document.getElementById("saMathLogicDetails");
            const summaryHTML = detailsContainer.querySelector(
                ".sa-analyze-score-summary",
            ).outerHTML;
            detailsContainer.innerHTML = summaryHTML + detailsHTML;
        ...
        async exportAnalysisAsImage() {
            ...
            ctx.beginPath();
            ctx.moveTo(
                188.28534 * scale + offsetX,
                196.60551 * scale + offsetY,
            );
            ctx.lineTo(
                188.28534 * scale + offsetX,
                (196.60551 - 68.25289) * scale + offsetY,
            );
            ctx.lineTo(
                (188.28534 + 33.21101) * scale + offsetX,
                (196.60551 - 68.25289) * scale + offsetY,
            );
            ctx.lineTo(
                (188.28534 + 33.21101) * scale + offsetX,
            ...
            ctx.lineTo(
                188.28534 * scale + offsetX,
                (196.60551 - 33.21101) * scale + offsetY,
            );
            ctx.lineTo(
                (188.28534 + 103.29475) * scale + offsetX,
                (196.60551 - 33.21101) * scale + offsetY,
            );
            ctx.lineTo(
                (188.28534 + 103.29475) * scale + offsetX,
            ...
            ctx.moveTo(
                188.28534 * scale + offsetX,
                231.64738 * scale + offsetY,
            );
            ctx.lineTo(
                188.28534 * scale + offsetX,
                (231.64738 - 33.21101) * scale + offsetY,
            );
            ctx.lineTo(
                (188.28534 + 68.32016) * scale + offsetX,
                (231.64738 - 33.21101) * scale + offsetY,
            );
            ctx.lineTo(
                (188.28534 + 68.32016) * scale + offsetX,
            ...
            ctx.lineTo(
                223.3945 * scale + offsetX,
                (161.56363 - 33.21101) * scale + offsetY,
            );
            ctx.lineTo(
                (223.3945 + 33.21101) * scale + offsetX,
                (161.56363 - 33.21101) * scale + offsetY,
            );
            ctx.lineTo(
                (223.3945 + 33.21101) * scale + offsetX,
                161.56363 * scale + offsetY,
            );
            ctx.closePath();
            ctx.fill();
            ...
            ctx.lineTo(
                (258.50366 + 33.21101) * scale + offsetX,
                (161.56363 - 33.21101) * scale + offsetY,
            );
            ctx.lineTo(
                (258.50366 + 33.21101) * scale + offsetX,
            ...
            ctx.lineTo(
                258.50366 * scale + offsetX,
                (231.64738 - 68.25289) * scale + offsetY,
            );
            ctx.lineTo(
                (258.50366 + 33.21101) * scale + offsetX,
                (231.64738 - 68.25289) * scale + offsetY,
            ...
            // 获取作品名称
            const projectName =
                this.projectName || projectJSON.info?.objName || "";
            ...
            if (projectName) {
                ctx.fillText(
                    msg("export-title", { name: projectName }),
                    logoX + 50,
                    logoY + 24,
                );
            } else {
                ctx.fillText(
                    "Simple Project Analyser Result",
                    logoX + 50,
                    logoY + 24,
                );
            }
            ...
            ctx.fillText(
                "Generated by Simple Project Analyser",
                width / 2,
                totalHeight - 15,
            );
        ...
        drawStatsSection(ctx, analysis, width, y, padding, sectionPadding) {
            ...
            const stats = [
                {
                    label: msg("total-blocks", "总积木数"),
                    value: analysis.totalBlocks,
                },
                {
                    label: msg("effective-blocks", "有效积木"),
                    value: analysis.effectiveBlocks,
                },
                {
                    label: msg("function-definitions", "函数定义"),
                    value: analysis.functionDefinitions,
                },
                {
                    label: msg("stat-sprites", "精灵数量"),
                    value: analysis.sprites,
                },
                {
                    label: msg("stat-costumes", "造型数量"),
                    value: analysis.costumeCount,
                },
                {
                    label: msg("stat-sounds", "声音数量"),
                    value: analysis.soundCount,
                },
                {
                    label: msg("stat-variables", "变量数量"),
                    value: analysis.variableCount,
                },
                {
                    label: msg("stat-lists", "列表数量"),
                    value: analysis.listCount,
                },
                {
                    label: msg("extensions", "扩展数量"),
                    value: analysis.extensions.length,
                },
            ];
        ...
        drawCodeTypeChart(ctx, analysis, width, y, padding, sectionPadding) {
            ...
            displayLabels.forEach((label, index) => {
                ...
                ctx.fillText(
                    `${displayLabel}: ${displayData[index]}`,
                    legendX + 14,
                    legendY + 9,
                );
            });
            ...
            if (labels.length > maxItems) {
                ...
                ctx.textAlign = "left";
                ctx.fillText(
                    `+${labels.length - maxItems} more...`,
                    legendStartX,
                    y + 50 + Math.ceil(maxItems / 3) * 26 + 10,
                );
            }
        ...
        drawDrScratchSection(ctx, scores, width, y, padding, sectionPadding) {
            ...
            ctx.textAlign = "left";
            ctx.fillText(
                msg("dr-scratch-score", "Dr.Scratch评分系统"),
                padding + 20,
                y + 30,
            );
            ...
            ctx.fillStyle = "#333333";
            ctx.fillText(
                `${msg("total-score", "总分")}: ${totalScore} / 21`,
                detailsX,
                detailsY,
            );
            detailsY += 25;
        ...
        drawMathLogicSection(ctx, scores, width, y, padding, sectionPadding) {
            ...
            ctx.textAlign = "left";
            ctx.fillText(
                msg("math-logic-assessment", "核心数学能力评估"),
                padding + 20,
                y + 30,
            );
            ...
            ctx.fillStyle = "#333333";
            ctx.fillText(
                `${msg("math-total-score", "数学总分")}: ${totalScore}`,
                detailsX,
                detailsY,
            );
            detailsY += 25;
        ...
        drawExtensionsSection(
            ...
            extensions.forEach((extension, index) => {
                ...
                ctx.textAlign = "left";
                const displayName =
                    extension.name.length > 16
                        ? extension.name.substring(0, 16) + "..."
                        : extension.name;
                ...
                ctx.textAlign = "right";
                ctx.fillText(
                    `${extension.blocks.length} ${msg("blocks-count", "blocks")}`,
                    itemX + itemWidth - 18,
                    itemY + 18,
                );
            });
    ...
    AddToBar(addon, {
        ...
        onDeactivate: () => {
            if (analyzer.chartInstance) {
                analyzer.chartInstance.destroy();
                analyzer.chartInstance = null;
            }
            if (analyzer.mathLogicChartInstance) {
                analyzer.mathLogicChartInstance.destroy();
                analyzer.mathLogicChartInstance = null;
            }
            if (analyzer.drScratchChartInstance) {
                analyzer.drScratchChartInstance.destroy();
                analyzer.drScratchChartInstance = null;
            }
        },
        onClick: () => {
