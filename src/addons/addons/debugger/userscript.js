...
export default async function ({ addon, console, msg }) {
  ...
  let compilerWarning_click=false
  compilerWarning.addEventListener("click", () => {
    if(compilerWarning_click){
      compilerWarning.textContent = msg("turn_debug");
      addon.tab.redux.dispatch({
        type: "scratch-gui/modals/OPEN_MODAL",
        modal: "settingsModal"
      }); 
      compilerWarning_click = false 
    }else{ 
      compilerWarning.textContent = msg("turn_debug_introduce"); 
      compilerWarning_click=true}
  });
