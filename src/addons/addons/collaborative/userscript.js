...
export default async function ({ addon, console, msg }) {
        ...
        while (retryNum < maxRetries) {
            ...
            try {
                const response = await fetchWithTimeout(
                    `http://${url}/roomIsFree?roomId=${encodeURIComponent(RoomId)}`,
                );
    ...
    async function fetchWithTimeout(url, timeout = 3000) {
        ...
        try {
            ...
            clearTimeout(timeoutId);
            console.error(error);
            updateTipText(msg(`fetch_failed`));
        }
    }
    ...
    const handleServerMessage = (msg) => {
        ...
        switch(msg.type){
            case 'connection':
                ...
                serverConfig.clientId = msg.clientId;
                serverConfig.roomId = msg.roomId;
        }
        // switch (msg.type) {
        //     case 'failed':
    ...
    const login = async () => {
        ...
        if (!allSTUN_URLs) {
            try {
                allSTUN_URLs = await fetchWithTimeout(
            ...
            allSTUN_URLs = await allSTUN_URLs.text();
        }
        ...
        server.onerror = (msgs) => {
            updateTipText(msg("server_error") + JSON.stringify(msgs), "error");
        };
        // 退出编辑器
